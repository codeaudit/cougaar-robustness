...
public class DisconnectManagerPlugin extends DisconnectPluginBase {
    ...
    private IncrementalSubscription agentExistsConditionSubscription;
    ...
    public void setupSubscriptions() {
        ...
        //Listen for the new Agents or moved Agents
        agentExistsConditionSubscription = (IncrementalSubscription ) getBlackboardService().subscribe( new UnaryPredicate() {
            public boolean execute(Object o) {
    ...
    public void execute() {
        ...
        if (managerAddress != null) {// already know the ManagerAgent, so create Diagnoes & Actions for newly announced Nodes & Agents
            ...
            iter = agentExistsConditionSubscription.getAddedCollection().iterator();
            while (iter.hasNext()) {
                AgentExistsCondition aec = (AgentExistsCondition)iter.next();
                RequestToDisconnectAgentDiagnosis diag = (RequestToDisconnectAgentDiagnosis) createDiagnosisAndAction(new AssetID(aec.getAsset(), AssetType.findAssetType("Agent")));
                if (diag.getValue().equals("Tardy")) {
                    try {
        ...
        if (i.hasNext()) { // just found the ManagerAgentAddress
            ...
            iter = agentExistsConditionSubscription.iterator();
            while (iter.hasNext()) {
                AgentExistsCondition aec = (AgentExistsCondition)iter.next();
                RequestToDisconnectAgentDiagnosis diag = (RequestToDisconnectAgentDiagnosis) createDiagnosisAndAction(new AssetID(aec.getAsset(), AssetType.findAssetType("Agent")));
                if (diag.getValue().equals("Tardy")) {
    ...
    private RequestToDisconnectNodeDiagnosis createNodeDiagnosisAndAction(AssetID id, String currentDiagnosis) {
        ...
        if (diag != null) { // the DefenseApplicability condition already exists, so don't make anotherset of conditions & opmodes
            if (logger.isDebugEnabled()) logger.debug("Not creating redundant Diagnosis & Action for already known "+id.toString() + ", ignoring");
            /* dlw - 9/29/04 - resetting kills off the saved Diagnosis state after MA restart
            try {
        ...
        try {
            ...
            if (currentDiagnosis.equals(DISCONNECTED)) { // then the action must be ACTIVE
                action.setValuesOffered(ALLOW_DISCONNECT_SET);
                ...
                action.setValuesOffered(NULL_SET); // nothing is offered
            }
    ...
    private void createOverdueAlarm(RequestToDisconnectNodeDiagnosis diag, AgentVector agents, double time) {
        ...
        blackboard.publishChange(nodeStatus);
        if (logger.isDebugEnabled()) logger.debug("Creating OverdueAlarm");
    }
