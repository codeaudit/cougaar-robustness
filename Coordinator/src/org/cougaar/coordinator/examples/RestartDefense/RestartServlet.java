...
package org.cougaar.coordinator.examples.RestartDefense;
...
/**
 * Servlet for Restart Defense
 */
public class RestartServlet extends ComponentPlugin
{
    private ServiceBroker sb;
    private LoggingService log;
    private ServletService servletSvc;

    RestartRawSensorData sensorData = null;
    RestartRawActuatorData actuatorData = null;
    MyServlet myServlet;
    ...
    {
	try {
	    servletSvc.register("/RestartDefense", new MyServlet());
	} catch (Exception e) {
	...
	while (iter.hasNext()) {
	    RestartRawSensorData data = (RestartRawSensorData)iter.next();
	    if (data != null) {
		sensorData = data;
	    }
	}
	...
	while (iter.hasNext()) {
	    RestartRawActuatorData data = (RestartRawActuatorData)iter.next();
	    if (data != null) {
		actuatorData = data;
	    }
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		if (val != null) { //the user updated the value
			...
			if (data != null) {
			    ...
			    break; // there's only one RestartRawSensorData object
			}
		    }
		...
		if (valuesOffered != null && valuesOffered.length > 0) { //the user updated valuesOffered
			...
			RestartRawActuatorData data = (RestartRawActuatorData)iter.next();
			if (data != null) {
			    data.setCommand(RestartRawActuatorData.SET_VALUES_OFFERED);
			    data.setValuesOffered(hs);
			    blackboard.publishChange(data);
			    valuesOfferedUpdated = true;
			    if (log.isDebugEnabled()) 
				log.debug(data + " changed."); 
			    break; // there's only one RestartRawActuatorData object
			}
		    }
		    blackboard.closeTransaction();
		}
		...
		if (actionValue != null) { //the user updated the value
			...
			RestartRawActuatorData data = (RestartRawActuatorData)iter.next();
			if (data != null) {
			    data.setCommand(RestartRawActuatorData.START);
		...
		if (code != null) { //the user updated the value
			...
			RestartRawActuatorData data = (RestartRawActuatorData)iter.next();
			if (data != null) {
			    data.setCommand(RestartRawActuatorData.STOP);
		...
		out.println("<h2>Restart Defense Servlet</h2>");
		...
		out.println("<hr>");
		out.println("<h3>Restart Diagnosis</h3>");
		Iterator iter = rawSensorDataSub.getCollection().iterator();
		while (iter.hasNext()) {
		    RestartRawSensorData data = (RestartRawSensorData)iter.next();
		    if (data != null) {
			...
			break; // there should be at most one RestartRawSensorData object
		    }
		}
		out.println("<hr>");
		out.println("<h3>Restart Action</h3>");
		...
		while (iter.hasNext()) {
			...
			break; // there should be at most one RestartRawActuatorData object
		    }
		}
		out.println("<hr>");
		out.println("</html>");
