...
public class RestartSensor extends ComponentPlugin
{
    private LoggingService log;
    private RestartDiagnosis diagnosis;
...
//	alarmService.addRealTimeAlarm(new DelayedStartAlarm(120000));
	...
	if (start == true) {
	    try {
		diagnosis = new RestartDiagnosis(agentId.toString(), sb);
		blackboard.publishAdd(diagnosis);
		if (log.isDebugEnabled()) 
		    log.debug(diagnosis + " added.");
		if (log.isDebugEnabled()) log.debug(diagnosis.dump());
		RestartRawSensorData data = new RestartRawSensorData(diagnosis.getAssetName(),
		...
		log.error("TechSpec not found for RestartDiagnosis", e);
	    }
	    start = false;
	}
