...
public class RestartActuator extends ComponentPlugin
{
    private LoggingService log;
    private RestartAction action;
    private RestartRawActuatorData data;
    ...
    private UnaryPredicate actionPred = new UnaryPredicate() {
		...
		return (o instanceof RestartAction);}};
...
//	alarmService.addRealTimeAlarm(new DelayedStartAlarm(120000));
	...
	if (start == true) {
		...
		log.error("TechSpec not found for RestartAction", e);
	    }
	    start = false;
	}
	...
	while (iter.hasNext()) {
		...
		switch (command) {
		case RestartRawActuatorData.SET_VALUES_OFFERED:
		    ...
		    break;
		case RestartRawActuatorData.START:
		    ...
		    break;
		case RestartRawActuatorData.STOP:
	...
	while (iter.hasNext()) {
	    RestartAction action = (RestartAction)iter.next();
	    if (action != null) {
		...
		if (newPV != null) {
			...
			data.setCommand(RestartRawActuatorData.SET_PERMITTED_VALUES);
