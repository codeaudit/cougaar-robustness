...
public class MetricsTestPlugin extends ServiceUserPlugin {
  ...
  private void setMetricsConditions() {
    ...
    Metric metric =  metricsService.getValue("Agent(MonitoredAgent1):LastHeard");
    if (metric != null) {
        long valueL = metric.longValue();
        if (logger.isInfoEnabled()) logger.info(valueL + " secs since last message rec'd from MonitoredAgent1");
    } else {
